...
class WordKey {
    ...
    init(word: String, startPos: Position, endPos: Position, direction: Direction) {
        self.word = word
    ...
    func getPositions() -> [(Int, Int)] {
        ...
        let step = self.direction.getMovingSteps()
        var row = startPos.row
        var col = startPos.col
        
        for _ in startPos.row...startPos.row+length {
...
class WordGridGenerator {
    ...
    private func add(word: String, startPos: Position, endPos: Position, direction: Direction) {
        wordsMap[WordGridGenerator.wordKey(for: startPos, and: endPos)] = word
        
        let wordKeyMap = WordKey(word: word, startPos: startPos, endPos: endPos, direction: direction)
        wordsKeyMap.append(wordKeyMap)
    }
        ...
        while true {
            ...
            if pos == nil {
                ...
                if let (grid, lastPos) = assignWord(grid: state.grid, word: word, direction: direction!, position: pos!) {
                    ...
                    let pos = Position(row: posRow, col: posCol)
                    
                    let dir = direction!
                    
                    add(word: word, startPos: pos, endPos: lastPos, direction: dir)
                }
            }
        }
